      ...
      <el-form label-position="top" :model="form" :rules="rules" ref="formEl">
        ...
        <el-row :gutter="24">
          ...
          <el-col :sm="24" style="text-align: center; padding-top: 50px">
            <el-button size="large" type="success" @click="onUpdate(formEl)"
              >
              <el-icon>
                <upload-filled/>
              </el-icon>
...
export default {
  ...
  setup(props) {
      ...
      await formEl.validate(async (valid, fields) => {
        if (valid) {
          ...
          try {
            ...
            const { data } = await axios.post(route("api.post.update",{post:props.id}), f);
            go({ name: "post.index" });
          } catch (error) {
            console.error(error);
          }
    ...
    axios.get(route("api.post.show", { post: props.id })).then(({ data }) => {
      const {
        body,
        categories,
        description,
      ...
      form.body = body;
      form.on_feature = on_feature
      form.on_home_top = on_home_top;
      form.on_popular = on_popular;
      form.on_trending = on_trending;
      form.show_author = show_author;
      form.description = description;
      form.title = title;
      form.categories = categories.map(e=>e.id)
      form.tags = tags.map(e=>e.title)
      images.value = attachments.map(e=>e.md_url);
