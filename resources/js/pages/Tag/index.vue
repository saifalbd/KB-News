<template>
    <app-layout :busy="busy">
        <div>
            <page-title-box title="Tags">
                <create-button title="Add Tag" @click="showCreate = true"></create-button>
            </page-title-box>
        </div>
     ...
     <el-table-column prop="is_popular" label="Popular">
        <template #default="{ row }">
          <el-switch
            ...
            @change="popularChange($event, row.slug)"
          />
        </template>
      </el-table-column>
...
export default {
    ...
    setup() {
      ...
      const url = route("api.tag.popularChange", { tag: id });
